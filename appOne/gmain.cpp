...
void gmain() {
    ...
    int ninjinImg = loadImage("assets\\ninjin0.png");
    ...
    while (notQuit) {
        ...
        for (int i = 0; i < num; i++) {
            ...
            if (sw == 0) {
                strokeWeight(Size * 10.0f);
                mathPoint(p[i].x / p[i].z, p[i].y / p[i].z);
            }
            else if (sw == 1) {
                mathImage(ufoImg, p[i].x / p[i].z, p[i].y / p[i].z, 0, Size);
            }
            else if (sw == 2) {
                mathImage(ninjinImg, p[i].x / p[i].z, p[i].y / p[i].z, p[i].angle, Size);
            }
        }
    }
}
